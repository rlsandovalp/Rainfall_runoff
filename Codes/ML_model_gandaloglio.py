...
svr_lin = SVR(kernel='linear', C=1, epsilon = 0)
svr_lin.fit(X_train, Y_train)
Y_predicted = svr_lin.predict(X_test)

np.savetxt('../Results/coefficients_gandaloglio_'+str(anticipation)+'hr.txt', svr_lin.coef_)
np.savetxt('../Results/intercept_gandaloglio_'+str(anticipation)+'hr.txt', svr_lin.intercept_)

plot_hydrograph(Y_predicted, Y_test)
plot_scatter(Y_predicted, Y_test, 0.1, 10)